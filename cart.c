...
void display_cart(ShoppingCart *cart, const char *username)
{
    ...
    printf("  No.  Product              Condition        Qty    Unit Price    Subtotal\n");
    printf("  ------------------------------------------------------------------------------\n");
    ...
    while (current != NULL)
    {
        printf("  [%-2d]  %-18s %-16s %-4d   PHP %-9.2f  PHP %.2f\n",
               item_number, current->item_title, current->condition, current->quantity,
               current->price, current->price * current->quantity);
        item_number++;
        current = current->next;
    }
    ...
    printf("\n");
    printf("  [C] Checkout   [R] Remove Item   [U] Update Quantity   [X] Clear Cart   [B] Back\n");
    printf("\n");
    printf("================================================================================\n");
}

/* --- Display cart item details dialog --- */
static void display_cart_item_details(const CartItem *item)
{
    printf("\n");
    printf("================================================================================\n");
    printf("                                CART ITEM DETAILS                              \n");
    printf("================================================================================\n");
    printf("  ID        : %d\n", item->item_id);
    printf("  Product   : %s\n", item->item_title);
    printf("  Condition : %s\n", item->condition);
    ...
    printf("================================================================================\n");
    printf("  [C] Checkout Now   [B] Back to Cart\n");
    printf("================================================================================\n");
    printf("  Enter choice [0-Back]: ");
}
    ...
    while (1)
    {
        ...
        else if (strcasecmp(input, "C") == 0)
        {
            if (cart->count > 1)
            {
                ...
                int cr = checkout_display(&temp_cart, username, stack, catalog, 1);
                if (cr == 1)
                {
        ...
        else if (strcasecmp(input, "R") == 0)
        {
            ...
            strncpy(removed_name, current->item_title, sizeof(removed_name) - 1);
...
quantity_loop:
            ...
            if (update_cart_item_quantity(cart, current->item_id, new_quantity))
            {
                printf("\n");
                printf("================================================================================\n");
                printf("                                    YOUR CART                                   \n");
...
int update_cart_item_quantity(ShoppingCart *cart, int item_id, int new_quantity)
{
    ...
    while (current != NULL)
    {
        if (current->item_id == item_id)
        {
            /* Update total by removing old quantity cost and adding new quantity cost */
...
static void print_checkout_step1_summary(const ShoppingCart *cart)
{
    ...
    while (cur != NULL)
    {
        printf("  [ %2d] %-19s %-12s %-4d   PHP %-9.2f  PHP %.2f\n",
               n, cur->item_title, cur->condition, cur->quantity, cur->price, cur->price * cur->quantity);
        n++;
        cur = cur->next;
    }
...
/* checkout_display: 0 = resume cart, 1 = order placed, 2 = leave cart menu (back to buyer menu) */
    ...
    if (catalog != NULL)
    {
        if (!suppress_unavailable_message)
        {
            ...
            while (chk != NULL)
            {
                ...
                if (it == NULL)
                {
                    it = get_item_by_details(catalog,
                                             chk->item_title,
                                             chk->condition,
                                             chk->seller,
                                             chk->contact,
        ...
        if (is_cart_empty(cart))
        {
            while (1)
            {
                ...
                printf("         Some items became unavailable or out of stock.\n");
                printf("\n");
                printf("================================================================================\n");
                printf("  [B] Browse Products   [M] Back to Buyer Menu\n");
                printf("================================================================================\n");
...
payment_select:
    ...
    while (1)
    {
        ...
        if (strcasecmp(payment_input, "B") == 0 || strcmp(payment_input, "0") == 0)
            return 0;
...
gcash_confirm:
        ...
        {
            snprintf(payment_record, sizeof(payment_record), "GCash (Ref: %s)", gcash_ref);
        }
...
confirm_order:
    ...
    {
        if (strcasecmp(co_nav, "N") == 0 || strcasecmp(co_nav, "B") == 0 || strcmp(co_nav, "0") == 0)
        {
            while (1)
            {
                printf("\n");
    ...
    while (1)
    {
        ...
        if (strcasecmp(order_nav, "V") == 0)
        {
            while (1)
            {
                ...
                if (strcasecmp(detail_nav, "B") == 0 || strcmp(detail_nav, "0") == 0)
                    break;
                if (strcasecmp(detail_nav, "V") == 0)
                {
                    ...
                    TxNode *selected_order = get_user_order_by_index(stack, username, detail_idx);
                    if (!selected_order)
                    {
                    ...
                    while (1)
                    {
                        ...
                        {
                            if (strcasecmp(selected_order->status, "Pending") != 0)
                            {
                                printf("================================================================================\n");
                                printf("                                CANNOT CANCEL ORDER                             \n");
                                printf("================================================================================\n");
                                printf("  Order #%05d is already %s.\n", selected_order->order_id, selected_order->status);
                                printf("  Only PENDING orders can be cancelled.\n");
                                printf("--------------------------------------------------------------------------------\n");
                                printf("  Enter choice [0-Back]: ");
                                char back_input[32];
                            ...
                            printf("                                  CANCEL ORDER                                  \n");
                            printf("================================================================================\n");
                            printf("  Are you sure you want to cancel Order #%05d?\n", selected_order->order_id);
                            printf("  Product : %s\n", selected_order->item_title);
                            printf("  Total   : PHP %.2f\n", selected_order->price);
                            printf("--------------------------------------------------------------------------------\n");
                            printf("                        [Y] Yes, Cancel   [N] No, Go Back                       \n");
                            printf("================================================================================\n");
                            printf("  Enter choice [0-Back]: ");
                            ...
                            char confirm_char = toupper((unsigned char)confirm_cancel[0]);
                            if (confirm_char == 'Y')
                            {
                                ...
                                printf("================================================================================\n");
                                printf("  Order #%05d has been successfully cancelled.\n", selected_order->order_id);
                                printf("================================================================================\n");
                                printf("  Enter choice [0-Back]: ");
                                char back_input[32];
                                fgets(back_input, sizeof(back_input), stdin);
